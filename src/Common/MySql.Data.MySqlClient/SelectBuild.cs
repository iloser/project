...
namespace MySql.Data.MySqlClient
{
    ...
    public class SelectBuild<TReturnInfo, TLinket> : SelectBuild<TReturnInfo> where TLinket : SelectBuild<TReturnInfo>
    {
        ...
        public new TLinket InnerJoin<TBLL>(string alias, string fields, string on)
        {
            return base.InnerJoin<TBLL>(alias, fields, on) as TLinket;
        }
        public new TLinket LeftJoin<TBLL>(string alias, string fields, string on)
        {
            return base.LeftJoin<TBLL>(alias, fields, on) as TLinket;
        }
        public new TLinket RightJoin<TBLL>(string alias, string fields, string on)
        {
            return base.RightJoin<TBLL>(alias, fields, on) as TLinket;
        }
    ...
    public class SelectBuild<TReturnInfo>
    {
        ...
        protected string _sort, _table, _join, _where, _groupby, _having;
        ...
        public string ToString(string field)
        {
            ...
            for (var i = 0; i < _dals.Count; i++)
            {
                if (_dals[i].Field.Count != 0 && i != 0)
                    fieldStr += ",";
                fieldStr += string.Join(",", _dals[i].Field);
            }
            string sql = string.Concat("SELECT ", field ?? fieldStr, _table, _join, where, _sort, limit);
            return sql;
        }
        /// <summary>
        /// 查询指定字段，返回元组或单值
        ...
        protected SelectBuild<TReturnInfo> Join<TBLL>(string alias, string fields,string on, string joinType)
        {
            IDAL dal = this.ConvertTBLL<TBLL>();
            dal.Field = new List<string>();
            var fieldList = fields.Split(',');
            foreach (var field in fieldList)
                dal.Field.Add(alias+".`"+field+"`");
            _dals.Add(dal);
        ...
        public SelectBuild<TReturnInfo> InnerJoin<TBLL>(string alias,string fields, string on)
        {
            return this.Join<TBLL>(alias,fields, on, "INNER JOIN");
        }
        public SelectBuild<TReturnInfo> LeftJoin<TBLL>(string alias, string fields, string on)
        {
            return this.Join<TBLL>(alias,fields, on, "LEFT JOIN");
        }
        public SelectBuild<TReturnInfo> RightJoin<TBLL>(string alias, string fields, string on)
        {
            return this.Join<TBLL>(alias,fields, on, "RIGHT JOIN");
        }
