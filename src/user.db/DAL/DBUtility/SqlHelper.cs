...
namespace user.BLL {
	...
	/// 数据库操作代理类，全部支持走事务
	/// </summary>
	public abstract partial class SqlHelper : user.DAL.SqlHelper {
	}
}
namespace user.DAL {
	public abstract partial class SqlHelper {
		...
		public static Executer Instance { get; } = new Executer(new LoggerFactory().CreateLogger("user_DAL_sqlhelper"), ConnectionString);
